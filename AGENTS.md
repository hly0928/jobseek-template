...
Workers must write all operational and process files in simple, concise English.
...
The lead and user decide when a batch is ready to close. Before running `complete-batch`, merge every completed discovery output, stop planning new scope, give intended jobs a clear working outcome, and archive confirmed applications. `complete-batch` reconciles every batch job's final assessment, audit, and archive outcome into `history/reviewed-jobs.jsonl`, rebuilds the reviewed-URL index, and then records the lifecycle change. It does not run live-profile preflight or re-audit business policy. Only the root lead runs it, and the next batch is not created until the current batch is completed.
